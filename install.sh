...
sudo tee -a /etc/apache2/sites-available/amazon-treasure-chest.conf <<VIRTUALHOST
<VirtualHost *:80>
	ServerName amazon-treasure-chest.localhost.com
...
echo "------------------ Make CERT. ------------------"
sudo openssl req -new -newkey rsa:4096 -days 3652 -nodes -x509 -subj "/C=US/ST=UT/L=Ogden/O=IT/CN=dev.atc.dustinwoodard.net" -keyout /var/www/server.pem  -out /var/www/server.cert

sudo a2ensite amazon-treasure-chest.conf
